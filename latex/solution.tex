...
All measurements were collected on one \textbf{TODO: GPU name and VRAM}
using vLLM \textbf{TODO: version}, CUDA \textbf{TODO: version}, and
PyTorch \textbf{TODO: version}. Each checkpoint was loaded and measured
...
\textbf{TODO after the Colab run: report the measured crossovers and ratios,
not an expected trend.} State where OLMoE is close to the 1B dense model
because only a subset of experts (about 1B parameters) is active per token.
Then state where it approaches the 7B dense model because all 7B parameters
must reside in memory and a batch that routes across many experts increases
expert-weight traffic and routing/kernel overhead. Discuss separately how
longer prefills change arithmetic intensity and how concurrent decode streams
change GPU utilization and the union of experts touched. These mechanisms are
consistent with the OLMoE architecture described by
\citet{muennighoff2025olmoe}, but the claimed serving advantage here must be
supported by the measured values in Figures~\ref{fig:prefill-throughput} and
\ref{fig:decode-throughput}.
